...
resource "azurerm_traffic_manager_profile" "mug-trmp-dev" {
  name                = "mug-trmp-dev"
  resource_group_name = "${azurerm_resource_group.mug-rg-infra.name}"
  ...
  dns_config {
    relative_name = "devmuginclermont"
    ttl           = "${var.traffic_manager_ttl}"
  }
...
resource "azurerm_traffic_manager_endpoint" "mug-trmp-endpoint-dev" {
  name                = "mug-trmp-endpoint-dev"
  resource_group_name = "${azurerm_resource_group.mug-rg-infra.name}"
  profile_name        = "${azurerm_traffic_manager_profile.mug-trmp-dev.name}"
  target_resource_id  = "${azurerm_public_ip.mug-public-ip.id}"
  type                = "azureEndpoints"
  weight              = 10
}
